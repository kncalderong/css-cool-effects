...
const SlidingImageTrack = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const constrainedPercentage = Math.max(
      Math.min(unconstrainedPercentage, 0),
      -1
    ) //to do not slide infinitely the track
    ...
    trackRef.current!.animate(
      {
        transform: `translate(${
          currentPercentage * trackRef.current!.scrollWidth
        }px, -50%)`,
      },
      {
        duration: 1200,
        ...
        {sliderImages.map((image, idx) => {
          return (
            <img
              src={image}
              draggable='false'
              className='w-[40vmin] h-[56vmin] object-cover object-center imageSlider'
              key={idx}
            ></img>
          )
        })}

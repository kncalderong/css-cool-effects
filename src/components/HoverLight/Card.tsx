...
const Card = ({ icon, title, content, hoverCards }: CardProps) => {
  ...
  return (
    <div className="card bg-[#ffffff1a] rounded-[10px] cursor-pointer flex flex-col relative w-full sm:w-[calc(50%-4px)] sm:flex-shrink h-[180px] lg:h-[260px] lg:w-[300px]"  onMouseOver={() => {setIsHover(true)}} onMouseLeave={() => { setIsHover(false) }}>
      ...
      <div className="card-content bg-dark-gray rounded-[10px] flex flex-col flex-grow inset-[1px] p-[10px] absolute z-[2]">
        <div className="card-image flex justify-center items-center h-[80px] lg:h-[140px] overflow-hidden">
          <FontAwesomeIcon icon={icon} className='text-[3em]  lg:text-[6em] opacity-25' color='#f0f0f0' />
        </div>
        <div className="card-info-wrapper flex flex-grow items-center justify-start py-0 px-[10px] lg:px-[20px]">
          <div className="card-info flex gap-[10px] justify-center items-start">
            ...
            <div className="card-info-title">
              <h3 className='text-[#f0f0f0] font-rubik text-[0.9em] lg:text-[1.1em]' >{title}</h3>
              <h4 className='text-[#ffffff80] font-rubik text-[0.8em] lg:text-[0.85em] mt-1 lg:mt-2'>{content}</h4>
            </div>
          </div>
        </div>
      </div>
